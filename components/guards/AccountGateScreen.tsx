...
export interface AccountGateScreenProps {
  title: string;
  message: string;
  detail?: string;
  onLogout: () => void;
...
export const AccountGateScreen: React.FC<AccountGateScreenProps> = ({
  title,
  message,
  detail,
  onLogout,
  ...
  <div className="relative flex h-screen flex-col items-center justify-center overflow-hidden bg-[var(--lp-void)] p-6 text-center text-[var(--lp-text)]">
    ...
    <div className="relative z-10 max-w-md space-y-6">
      ...
      <h1 className="font-display text-3xl font-bold text-red-300">{title}</h1>
      <p className="text-[var(--lp-muted)]">{message}</p>
      {detail ? (
        <p className="rounded-xl border border-[var(--lp-border)] bg-[var(--lp-surface)] px-4 py-3 text-left text-sm text-[var(--lp-muted)]">
          {detail}
        </p>
      ) : null}
      <button
